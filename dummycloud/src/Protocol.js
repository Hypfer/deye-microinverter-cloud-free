...
class Protocol {
  static parseHeader(buf) {
    ...
    if (result.payloadLength + HEADER_LEN + FOOTER_LEN !== buf.length) {
      throw new Error(
        "Payload length from header doesn't match packet length. Truncated?",
      );
    }
  ...
  static parsePacket(buf) {
    ...
    if (typeStr) {
      ...
      Logger.warn(
        `Received packet of unknown type "0x${header.type.toString(16)}"`,
      );
    }
    ...
    return {
      pv: {
        1: {
          ...
          w: parseFloat(
            (
              (packet.payload.readUInt16LE(85) / 10) *
              (packet.payload.readUInt16LE(87) / 10)
            ).toFixed(2),
          ),
        ...
        2: {
          ...
          w: parseFloat(
            (
              (packet.payload.readUInt16LE(89) / 10) *
              (packet.payload.readUInt16LE(91) / 10)
            ).toFixed(2),
          ),
        ...
        3: {
          ...
          w: parseFloat(
            (
              (packet.payload.readUInt16LE(93) / 10) *
              (packet.payload.readUInt16LE(95) / 10)
            ).toFixed(2),
          ),
        ...
        4: {
          ...
          w: parseFloat(
            (
              (packet.payload.readUInt16LE(97) / 10) *
              (packet.payload.readUInt16LE(99) / 10)
            ).toFixed(2),
          ),
      ...
      inverter_meta: {
        ...
        grid_freq_hz_overfreq_load_reduction_starting_point:
          packet.payload.readUInt16BE(223) / 100,
  ...
  static parseLoggerPacketPayload(packet) {
    //TODO: there's a lot more in this packet
    return {
      fw_ver: truncateToNullTerminator(
        packet.payload.subarray(19, 60).toString("ascii"),
      ),
      ip: truncateToNullTerminator(
        packet.payload.subarray(65, 82).toString("ascii"),
      ),
      ver: truncateToNullTerminator(
        packet.payload.subarray(89, 130).toString("ascii"),
      ), //hw revision maybe?
      ssid: truncateToNullTerminator(
        packet.payload.subarray(172, 210).toString("ascii"),
      ),
    };
  }
  ...
  static buildTimeResponse(packet) {
    ...
    response.writeUint32LE(Math.round(Date.now() / 1000), 13);
    response.writeUint32LE(0, 17);
