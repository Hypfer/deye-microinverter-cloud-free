...
class MqttClient {
  ...
  initialize() {
    ...
    if (process.env.MQTT_USERNAME) {
      ...
      Logger.error(
        "MQTT_PASSWORD is set but MQTT_USERNAME is not. MQTT_USERNAME must be set if MQTT_PASSWORD is set.",
      );
      process.exit(1);
    }
  ...
  handleData(data) {
    this.ensureAutoconf(
      data.header.loggerSerial.toString(),
      data.payload.inverter_meta.mppt_count,
    );
    ...
    for (let i = 1; i <= data.payload.inverter_meta.mppt_count; i++) {
      this.client.publish(
        `${baseTopic}/pv/${i}/v`,
        data.payload.pv[`${i}`].v.toString(),
      );
      this.client.publish(
        `${baseTopic}/pv/${i}/i`,
        data.payload.pv[`${i}`].i.toString(),
      );
      this.client.publish(
        `${baseTopic}/pv/${i}/w`,
        data.payload.pv[`${i}`].w.toString(),
      );
      this.client.publish(
    ...
    this.client.publish(
      `${baseTopic}/grid/active_power_w`,
      data.payload.grid.active_power_w.toString(),
    );
    ...
    this.client.publish(
      `${baseTopic}/grid/hz`,
      data.payload.grid.hz.toString(),
    );

    this.client.publish(
      `${baseTopic}/inverter/radiator_temperature`,
      data.payload.inverter.radiator_temp_celsius.toString(),
    );
  }
  ...
  ensureAutoconf(loggerSerial, mpptCount) {
    ...
    if (
      Date.now() - (this.autoconfTimestamps[loggerSerial] ?? 0) <=
      4 * 60 * 60 * 1000
    ) {
      return;
    }
    ...
    const device = {
      ...
      identifiers: [`deye_dummycloud_${loggerSerial}`],
    };
